...
class Labeller(object):
    ...
    def num_correct_paths(self, line):
        correct = 0
        min_length = 4
        for path in self.correct_path_generate(line):
            min_len_path = 3 - np.where(path == 0)[0].shape[0]
            if min_len_path < min_length:
                min_length = min_len_path

            correct += 1

        if correct <= 50:
            return 0, min_length
        elif correct > 50 and correct <= 100:
            return 1, min_length
        elif correct > 100 and correct <= 200:
            return 2, min_length
        elif correct > 200 and correct <= 500:
            return 3, min_length
        elif correct > 500:
            return 4, min_length
