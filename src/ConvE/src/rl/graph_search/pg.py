...
class PolicyGradient(LFramework):
    def __init__(self, args, kg, pn, logger):
        super(PolicyGradient, self).__init__(args, kg, pn, logger)
        ...
        if self.supervised_learning_mode:
            self.sl_loss = torch.nn.CrossEntropyLoss()
    ...
    def loss(self, mini_batch):
        ...
        final_reward_avg = sum(final_reward)/len(final_reward)
        print("{} reward avg is {}".format("SL" if self.supervised_learning_mode else "RL", sum(final_reward)/len(final_reward)))
        ...
        if self.supervised_learning_mode:
            ...
            for i in range(self.num_rollout_steps):
                ...
                step = sl_data[i]
                for bucket, (action_dist, label) in enumerate(step):
                    action_dist = action_dist.to(device=torch.device('cuda'), dtype=torch.float)
                    # in the cases where computing the label would cause the program to crash because of memory restrictions we simply dont
                    # and return none instead. In this case, we just say the cross entropy loss is 0
                    if label == None:
                        if bucket in step_totals:
                            step_totals[bucket] += self.sl_loss(action_dist, action_dist)
                        else:
                            step_totals[bucket] = self.sl_loss(action_dist, action_dist)
                    else:
                        label = label.to(device=torch.device('cuda'), dtype=torch.float)
                        if bucket in step_totals:
                            step_totals[bucket] += self.sl_loss(action_dist, label)
                        else:
                            step_totals[bucket] = self.sl_loss(action_dist, label)
            step_totals_tensor = torch.empty(len(step_totals.keys()), device=torch.device('cuda'))
            for key in step_totals:
                step_totals_tensor[key] = step_totals[key]
            
            sl_loss = torch.mean(step_totals_tensor)

            # entropy regularization
            sl_loss = (sl_loss - entropy * self.beta).mean()
            sl_p_loss = sl_loss.clone().detach()
        else:
            # Compute policy gradient loss
            pg_loss, pt_loss = 0, 0
            for i in range(self.num_rollout_steps):
        ...
        loss_dict = {}
        if self.supervised_learning_mode:
            loss_dict['model_loss'] = sl_loss
            loss_dict['print_loss'] = float(sl_p_loss)
        else:
            loss_dict['model_loss'] = pg_loss
            loss_dict['print_loss'] = float(pt_loss)
        loss_dict['reward'] = final_reward
        loss_dict['reward_avg'] = final_reward_avg
    ...
    def rollout(self, e_s, q, e_t, num_steps, visualize_action_probs=False):
        ...
        # print("Initial stats: {:.0%} reached goal".format(torch.count_nonzero(path_trace[-1][1] == e_t)/path_trace[-1][1].size(0)))
        sys.stdout.flush()
        # fetching the correct labels to compare the sample label to goes here
        for t in range(num_steps):
            last_r, e = path_trace[-1]
            obs = [e_s, q, e_t, t==(num_steps-1), last_r, seen_nodes]
            db_outcomes, inv_offset, policy_entropy = pn.transit(
                e, obs, kg, use_action_space_bucketing=self.use_action_space_bucketing, step=t, sl=self.supervised_learning_mode)
            sample_outcome = self.sample_action(db_outcomes, inv_offset)
            # print("{:.0%} reached goal".format(torch.count_nonzero(sample_outcome['action_sample'][1] == e_t)/sample_outcome['action_sample'][1].size(0)))

            # get the data to compute the loss
            if self.supervised_learning_mode:
                sl_labels.append(sample_outcome['sl_data'])
            else:
    ...
    def sample_action(self, db_outcomes, inv_offset=None):
        ...
        def sample(action_space, action_dist):
            ...
            if not self.supervised_learning_mode:
            ...
            sample_outcome['sample_dist'] = sample_action_dist
        ...
        if inv_offset is not None:
            ...
            action_dist_list = []
            action_prob_list = []
            # iterate over the buckets and take the relevant samples
            if self.supervised_learning_mode:
                sl_labels = []
                for (action_space, action_dist, label) in db_outcomes:
                    ...
                    sl_labels.append((action_dist, label))
            else:
            ...
            sample_outcome['action_prob'] = action_prob
            if self.supervised_learning_mode:
                sample_outcome['sl_data'] = sl_labels
        else:
