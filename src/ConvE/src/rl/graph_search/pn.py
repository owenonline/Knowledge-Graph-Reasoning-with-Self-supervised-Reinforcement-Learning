...
class GraphSearchPolicy(nn.Module):
    ...
    def transit(self, e, obs, kg, use_action_space_bucketing=True, merge_aspace_batching_outcome=False, step=0, sl=False):
        """
        ...
        if use_action_space_bucketing:
            ...
            references = []
            if sl:
                db_action_spaces, db_references, db_labels = self.get_action_space_in_buckets(e, obs, kg, sl, step=step)

                for action_space_b, reference_b, label_b in zip(db_action_spaces, db_references, db_labels):
                    X2_b = X2[reference_b, :]
                    action_dist_b, entropy_b = policy_nn_fun(X2_b, action_space_b)
                    references.extend(reference_b)
                    db_outcomes.append((action_space_b, action_dist_b, label_b))
                    entropy_list.append(entropy_b)
            else:
                db_action_spaces, db_references = self.get_action_space_in_buckets(e, obs, kg, sl)
    ...
    def get_correct_actions(self, e_space_b, e1, q, e2, mask, start_depth, max_depth, kg, last_seen_entities):
        # start depth at 1
        goal = torch.unsqueeze(e2, 1).repeat(1,e_space_b.size(dim=1))
        # assess which actions lead to the goal
        met_goal = torch.where(torch.logical_and(e_space_b == goal, mask), torch.full(e_space_b.size(), 1, device=torch.device('cuda')), torch.full(e_space_b.size(), 0, device=torch.device('cuda')))
        # recursively go deeper if needed
        if start_depth != max_depth:
            non_met_indices = torch.nonzero(met_goal == 0, as_tuple=True)
            query_rows = non_met_indices[0]
            new_e_batch = e_space_b[non_met_indices]
            obs_n = [e1[query_rows], q[query_rows], e2[query_rows], False]
            # handle memory errors
            try:
                next_step = self.get_correct_actions_recursive(new_e_batch, obs_n, start_depth + 1, start_depth, max_depth, kg)
            except Exception as e:
                print("Error: batch caused error for e set at depth {} with {} entities\nError message:\n{}\n\nClearing cache and skipping label...".format(start_depth, new_e_batch.size(0), str(e)))
                torch.cuda.empty_cache()
                return None
            next_step_overall = torch.any(next_step, dim=1)
            met_goal[non_met_indices] = next_step_overall.to(torch.long)
        # if we don't find any actions because the agent got itself into a bad spot, mark the actions that cause the agent to go back a step
        last_seen_entities = last_seen_entities.repeat(1, e_space_b.size(dim=1))
        # print(met_goal.size())
        # print(e_space_b.size())
        # print(last_seen_entities.size())
        backtrack_mask = (e_space_b == last_seen_entities.view(e_space_b.size())).to(torch.long)
        met_goal[torch.nonzero(torch.sum(met_goal, 1) == 0, as_tuple=True)] = backtrack_mask[torch.nonzero(torch.sum(met_goal, 1) == 0, as_tuple=True)]
        return met_goal

    def get_correct_actions_recursive(self, e, obs, depth, start_depth, max_depth, kg):
        e1, q, e2, last_step = obs

        entity2bucketid = kg.entity2bucketid[e]
        key1 = entity2bucketid[:, 0] # bucket
        key2 = entity2bucketid[:, 1] # position within bucket
        batch_ref = {}

        for i in range(len(e)):
            key = int(key1[i])# bucket that e1[i] is stored in
            if not key in batch_ref:
                batch_ref[key] = []
            batch_ref[key].append(i) # putting e1 indices in a dict with their bucket as the key

        met_goals_bucket = []
        met_goals_references = []

        # iterating over buckets
        for key in batch_ref:
            # get the bucket and the lists of indices we need to do everything
            action_space = kg.action_space_buckets[key] # bucket with id key
            l_batch_refs = batch_ref[key] # list of e1 indices in a specific bucket; e1[l_batch_refs] => list of e1s in the bucket
            g_bucket_ids = key2[l_batch_refs].tolist() # position of all e1s witin that bucket

            # get action spaces for each e in the batch, as well as the e's themselves
            e_b = e[l_batch_refs]
            r_space_b = action_space[0][0][g_bucket_ids]
            e_space_b = action_space[0][1][g_bucket_ids]
            action_mask_b = action_space[1][g_bucket_ids]

            # get the queries corresponding to each e in batch
            query_e1s = e1[l_batch_refs]
            query_rs = q[l_batch_refs]
            query_e2s = e2[l_batch_refs]
            goal = torch.unsqueeze(query_e2s, 1).repeat(1,e_space_b.size(dim=1)) # same size as actions so we can compare

            # get mask
            obs_b = [query_e1s, query_rs, query_e2s, last_step, [], []]
            ((r_space_b, e_space_b), mask) = self.apply_action_masks(((r_space_b, e_space_b), action_mask_b), e_b, obs_b, kg)

            # determine which actions for each e immediately result in a reward
            met_goal = torch.where(torch.logical_and(e_space_b == goal, mask), torch.full(e_space_b.size(), 1, device=torch.device('cuda')), torch.full(e_space_b.size(), 0, device=torch.device('cuda')))

            # remove the actions that have been masked out
            # met_goal[torch.nonzero(1 - mask, as_tuple=True)] = 0

            if depth < max_depth:
                non_met_indices = torch.nonzero(met_goal == 0, as_tuple=True)
                # if non_met_indices[0].size(0) > 3000:
                #     selected = torch.randint(low=0, high=non_met_indices[0].size(0), size=(3000,), device=torch.device('cuda'))
                #     non_met_indices = (non_met_indices[0][selected], non_met_indices[1][selected])
                query_rows = non_met_indices[0]

                # get the list of entities that are the result of taking each action in this level that
                # doesn't result in getting the reward, as well as the list of rs taken to get there, path so far,
                # and query in the same order
                new_e_batch = e_space_b[non_met_indices] # all the es that are the result of an action that didn't immediately earn a reward

                obs_n = [query_e1s[query_rows], query_rs[query_rows], query_e2s[query_rows], False]
                # gets back a matrix with a row for each new e value containing all the actions that can lead to 
                # a reward. We just want to know if the action that led to that new e2 can, so we reduce
                # the dimensionality with a logical any and then reshape the result to match this depth's met_goal
                # and use a logical or to update the valid actions
                next_step = self.get_correct_actions_recursive(new_e_batch, obs_n, depth + 1, start_depth, max_depth, kg)
                next_step_overall = torch.any(next_step, dim=1)
                met_goal[non_met_indices] = next_step_overall.to(torch.long)

            met_goals_bucket.append(met_goal)
            met_goals_references.extend(l_batch_refs)

        # merge buckets to that the higher level has the expected output
        inv_offset = [i for i, _ in sorted(enumerate(met_goals_references), key=lambda x: x[1])]
        return ops.pad_and_cat(met_goals_bucket, padding_value=0)[inv_offset]

    def get_action_space_in_buckets(self, e, obs, kg, sl, collapse_entities=False, step=0):
        """
        ...
        :return db_action_spaces:
            [((r_space_b0, e_space_b0), action_mask_b0),
             ((r_space_b1, e_space_b1), action_mask_b1),
             ...
             ((r_space_bn, e_space_bn), action_mask_bn)]
        ...
        if collapse_entities:
            ...
            for key in batch_ref:
                ...
                action_space_b = self.apply_action_masks(action_space_b, e_b, obs_b, kg, sl=sl)
                if sl:
                    correct_label = self.get_correct_actions(e_space_b, e_s_b, q_b, e_t_b, action_space_b[1], step, 2, kg, seen_nodes_b[:, -1])
                    db_labels.append(correct_label)
                db_action_spaces.append(action_space_b)
                db_references.append(l_batch_refs)
        if sl:
            return db_action_spaces, db_references, db_labels
        else:
    ...
    def apply_action_masks(self, action_space, e, obs, kg, sl=False):
        (r_space, e_space), action_mask = action_space
        e_s, q, e_t, last_step, last_r, seen_nodes = obs
